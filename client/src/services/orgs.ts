...
export const createOrg = async (org: Org): Promise<OrgRef> => {
  ...
  const req_result = await axios
    ...
    .then((res: CreateOrgResponse) => {
      console.log({ res });
      return "_id" in res && "org_id" in res
        ? { _id: res._id, org_id: res.org_id }
        : null;
    })
